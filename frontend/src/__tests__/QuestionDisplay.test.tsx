...
describe('QuestionDisplay', () => {
  ...
  it('renders all 9 question cards by default (all sections expanded)', () => {
    render(<QuestionDisplay questions={QUESTIONS} />)
    // Each card shows its question text (rendered as "1. T1" so we use regex)
  ...
  it('collapses a section when its header button is clicked', () => {
    ...
    // Technical questions should no longer be visible (T1 only appears in tech cards)
    expect(screen.queryByText(/T1-FU/)).not.toBeInTheDocument()
    // Other sections remain visible
    expect(screen.queryByText(/B1-FU/)).toBeInTheDocument()
  })
  ...
  it('re-expands a section when its header is clicked again', () => {
    ...
    expect(screen.queryByText(/T1-FU/)).toBeInTheDocument()
  })
