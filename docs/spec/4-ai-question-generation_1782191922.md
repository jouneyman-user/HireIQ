...
> **Branch:** `issue-4-ai-question-generation_1782191922`
> **Date:** 2026-06-23
> **Effort:** L (Large)
...
### Step 2 — Question Generator Service (`backend/app/services/claude_service.py`)
...
| `backend/app/services/claude_service.py` | **New** | Claude API call, prompt engineering, JSON parsing, validation |
...
- **Model selection:** `claude-3-5-haiku-20241022` is the recommended default for speed and cost at M2. Swap to `claude-sonnet-4-5` if output quality requires it — just change the string in `claude_service.py`.
...
- **Persistence:** Generated question sets are not stored in the database at M2. Add a `question_sets` table linked to `resumes` in M3 if history/replay is required.
...
*Spec generated autonomously by the Super Skills brainstorming agent — 2026-06-23.*
